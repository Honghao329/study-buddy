...
import {
  ArrowLeft, Heart, Bookmark, MessageCircle, Send, Eye, Share2,
} from 'lucide-react';
  ...
  if (!note) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-gray-50">
        <div className="text-gray-400 text-sm mb-4">笔记不存在或已删除</div>
    ...
    <div className="flex-1 flex flex-col bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto pb-20">
        ...
        <div className="flex items-center justify-around bg-white mt-2 py-3 border-y border-gray-100">
          <button
            className={`flex items-center space-x-1.5 px-4 py-2 rounded-full text-sm font-medium transition ${
          ...
          <button
            className={`flex items-center space-x-1.5 px-4 py-2 rounded-full text-sm font-medium transition ${
              favorited ? 'bg-yellow-50 text-yellow-600' : 'bg-gray-50 text-gray-500'
            }`}
            onClick={toggleFav}
          >
        ...
        <div className="px-5 pt-4 pb-4">
          ...
          {comments.length === 0 ? (
            <div className="text-center py-10 text-gray-400 text-sm">暂无评论，快来抢沙发~</div>
          ) : (
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-white border-t border-gray-100 px-4 py-3 flex items-center space-x-3 z-30">
        <input
          className="flex-1 bg-gray-100 rounded-full px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-200 transition placeholder-gray-400"
          placeholder="写评论..."
          value={commentText}
        ...
        <button
          className={`w-10 h-10 rounded-full flex items-center justify-center transition ${
            commentText.trim() ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-400'
          }`}
